...
overhead_runtime_wk_old = list(map(lambda (x,r): x * r, zip(runtime_wk_old, overhead_wk_old)))
native_runtime_wk_old = list(map(lambda (x,r): x * (1-r), zip(runtime_wk_old, overhead_wk_old)))
...
ax1.bar(ncores, native_runtime_wk_old, width, color = 'white', edgecolor="k", label="native")
ax1.bar(ncores, overhead_runtime_wk_old, width, color = 'black', bottom = native_runtime_wk_old, label="overhead")
...
for (i, (overhead, native)) in enumerate(zip(overhead_wk_old, native_runtime_wk_old)):
    plt.text(i+1, native + 0.05, "{:2.0f}x".format(overhead), color="white")
